...
from unet import unet_3156
from dataset import get_dataset
from utils import (
    save_parameters,
    load_parameters,
)
from unet_utils import DiceBCELoss, get_dataset_unet
''' Micro-Batch Streaming '''
from mbs import MicroBatchStreaming, MBSSegmentation
...
def validation_unet(
    ...
    acc = (sum(test_dices)/len(test_dices)) * 100
    valid_loss = (sum(test_losses)/len(test_losses))
    print(
            f"valid loss : {valid_loss:.2f}",
            f"acc: {acc:.2f} %"
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', type=int, default=1000000, help='batch size for train dataloader')

    ''' Dataloader, Optimizer etc. Arguments '''
    parser.add_argument('-unet', action='store_true', default=False, help='use gpu or not')
    ...
    if args.wandb:
        ...
        if args.unet:
            tags.append( f'scale 1/{args.scale}')
    ...
    #            )
        ...
        image_factor = 1 / args.scale
        carvana_dataset = get_dataset_unet(
    ...
    print(f">>> Type of Dataset: {train_dataset.__class__}")
    ...
    if args.unet:
        if (args.mbs or args.mbs_bn):
            for epoch in range(1, settings.EPOCH + 1):
                ...
                valid_loss, acc = validation_unet(
                    device=device,
                    model=net,
            ...
            for epoch in range(1, settings.EPOCH + 1):
                train_loss, train_time = train_unet(
