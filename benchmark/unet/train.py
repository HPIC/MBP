...
def train(epoch):
    ...
    start = time.time()
    net.train()
    for batch_index, (images, labels) in enumerate(carvana_training_loader):
        ...
        n_iter = (epoch - 1) * len(carvana_training_loader) + batch_index + 1
...
def eval_training(epoch=0, tb=True):
    ...
    for (images, labels) in carvana_test_loader:
    ...
    print('Test set: Epoch: {}, Average loss: {:.4f}, Accuracy: {:.4f}, Time consumed:{:.2f}s'.format(
        epoch,
        test_loss / len(carvana_test_loader.dataset),
        correct.float() / len(carvana_test_loader.dataset),
        finish - start
    ))
    print()
    ...
    return correct.float() / len(carvana_test_loader.dataset)
